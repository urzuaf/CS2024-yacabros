<script>
  ...
  async function deshabilitarEdicion() {
    ...
    try {
      const resp = await fetch(
        "http://localhost:3000/addInfoColClasificatoria",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      {#if showColumnIndexInput}
        <div class="mt-2">
          <input
            ...
            type="number"
            min="1"
            placeholder="Índice de la columna"
            class="px-2 ml-2 mt-2 bg-sportify h-10 w-16 text-white rounded-lg hover:text-gray-200"
          />
          <button
            ...
            on:click={() => {
              ...
              if (
                !isNaN(colIndex) &&
                colIndex > 0 &&
                colIndex <= document.querySelectorAll("#cabecera th").length
              ) {
